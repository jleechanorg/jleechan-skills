...
- Offer the four recovery paths: log-in-once-in-Chrome (then poll cron), manual download, vendor support, alt data source (see "Gmail-as-data-fallback" below).
...
Before concluding "I can't get the data," check whether the target site's data is **already emailed to the user**. Many fintech / SaaS tools (Venmo, Stripe, Square, PayPal, Shopify, banks) send monthly activity statements as email attachments or inline tables.
...
Worked example (2026-07-14, Venmo task): the auditor asked for Jan 2024 statements but Venmo only emails monthly histories back ~18 months, so Gmail had nothing for the actual target window. Saved me from going further down the wrong path.
