...
The loop runs autonomously until one of these is met:
1. User explicitly says "stop" or "pause"
2. 12 hours elapsed since first cycle
3. Context window > 90% consumed (use `/checkpoint` before hitting the limit)
4. System is 100% stable for 3 consecutive healthy cycles (all canary checks pass, 0 warnings, 0 lock issues, 0 orphan processes)
...
**6a.** Use `/claw` for each actionable bead. ALWAYS include `/er` and `/learn` in the task — every worker must validate evidence and record learnings:
```bash
...
**6c.** Run `/er` on items approaching full-green — validate evidence NOW, don't wait for a worker. If `/er` finds issues: fix the item's metadata/body directly and push a new commit if code changes are needed. Do NOT wait for a worker — fix it yourself.
