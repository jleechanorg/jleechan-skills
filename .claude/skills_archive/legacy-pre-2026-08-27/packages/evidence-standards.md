...
## Cross-Reference: Repo-Level Evidence Standards

This file is the **user-scope** (generic, cross-project) evidence standard.

**When working in a repo that has `.claude/skills/evidence-standards.md`:** Always
load the repo-level file too. Repo-specific rules take precedence over this file
when they conflict. For example, Your Project's repo-level `/es` bans
unit-test-only evidence for `$PROJECT_ROOT/` changes — that rule overrides the generic
Terminal/Backend class guidance below.

## Evidence Class System

Evidence requirements vary by project type. Choose the class that matches your project:

| Class | Examples | Required Evidence |
|-------|----------|-------------------|
| **Terminal/Backend** | APIs, services, CLIs, scripts | `run.json`, `metadata.json`, `evidence.md` (git provenance + test output sufficient) |
| **Integration** | DB-backed services, external API clients | Terminal class + `request_responses.jsonl` + server runtime provenance |
| **LLM/API** | LLM integrations, model fine-tuning, API pipelines | Integration class + `llm_request_responses.jsonl` |
| **UI/Browser** | Web apps, rendered interfaces, user flows | Terminal class + UI/browser video evidence |

For mixed-class projects, apply all applicable requirements. If your project has no browser UI, skip the browser video requirements. If your project has no LLM layer, skip the LLM request/response capture.

**Evidence is NOT one-size-fits-all.** For standalone CLIs, scripts, or libraries
with no server component, `pytest` output and git SHA may suffice. **However,
check the repo-level `/es` first** — many repos (e.g., Your Project) require
real server + real LLM evidence even for backend-only changes, making unit-test-only
evidence invalid. Do not generate video evidence for terminal-only work unless the
work specifically requires it.
...
def capture_provenance():
    ...
    # For GitHub/GitLab with remotes: fetch first to ensure refs are current
...
| `request_responses.jsonl` | Required for integration/LLM-class bundles or base-class local-server traces | Full request/response pairs |
| `llm_request_responses.jsonl` | Required for LLM/API-class bundles or base-class local-server traces | `type` field (`request` or `response`) |
...
|----------|----------|---------------------|
| **Test-only** | `*_test.py`, `*_test.ts`, `tests/`, `testing_mcp/`, `__tests__/` | Tests exercise existing behavior; they don't change it |
| **Docs-only** | `*.md`, `docs/`, `README` updates, `CLAUDE.md` | Documentation describes behavior; it doesn't alter it |
| **CI/workflow** | `.github/workflows/*.yml`, `Makefile`, lint configs | CI infrastructure doesn't change production behavior |
...
3. **Classify each changed file** — if every file matches one of the categories above, the existing evidence is still valid.
...
Diff SHA→HEAD: <N> file(s) changed — all test/docs/CI-only
...
- Any change to API endpoints, game logic, agent routing, or LLM prompts
...
many test iterations were added on top.
...
## Video Evidence — Conditional by Project Type
