...
## BigQuery Forensic Telemetry Verification

When capturing or auditing BigQuery telemetry evidence for backend or LLM lifecycle claims:

1. **`llm_forensics.log_events` Schema & Queries**:
   - Primary timestamp column: `ingested_at`
   - Emitted directive events: `directive_added`, `directive_dropped`, `directive_rejected`, `directive_ignored`
   - Standard query pattern:
     ```sql
     SELECT ingested_at, event_type, campaign_id, fields_json
     FROM `worldarchitecture-ai.llm_forensics.log_events`
     WHERE event_type IN ("directive_added", "directive_dropped")
     ORDER BY ingested_at DESC
     LIMIT 10;
     ```

2. **`llm_forensics.llm_payloads` Schema & Queries**:
   - Column `rag_mode` must be populated with `"god"` for `agent = 'GodModeAgent'` (NULL value indicates un-migrated or missing cohort backfill).
   - Standard query pattern:
     ```sql
     SELECT ingested_at, campaign_id, agent, rag_mode
     FROM `worldarchitecture-ai.llm_forensics.llm_payloads`
     WHERE agent = "GodModeAgent"
     ORDER BY ingested_at DESC
     LIMIT 10;
     ```
