...
6. **Execute advice reviews (`/advice` and conditional `/web-advice`):**
   ...
   - Run `/advice` on both the design specification and implementation plan. Without that separate authorization, pass this explicit parent constraint in the `/advice` request: `Reviewer D /web-advice is disabled; do not invoke it or any external browser transport.` Verify that the synthesis lists Reviewer D as `unavailable (disabled by parent authorization boundary)`. Treat an omitted row or an attempted Reviewer D/browser submission as an incomplete `/advice` run and record it as `FAILED`.
   - Retry `/advice` once only for a transient transport or reviewer-launch failure. If the retry also fails, record `/advice` as `FAILED` with both failure reasons and continue to the terminal report. A `FAILED` state permits the terminal report but prohibits claiming that advice passed or approved the documents. A returned `APPROVED`, `NOT APPROVED`, or `WITHHELD` synthesis counts as `RAN`; report that exact verdict without rewriting it.
   - `/web-advice` is applicable only when the user's current request separately and explicitly authorizes submitting the documents to external browser reviewers. If `/advice` already returned a Reviewer D `/web-advice` verdict covering both documents, reuse that result. Do not run a second standalone `/web-advice`. Otherwise run one standalone `/web-advice` review.
   - If browser authentication or transport is unavailable, record `/web-advice` as `UNAVAILABLE` and continue. Do not pause or ask the user to log in. Without explicit external-review authorization, record it as `SKIPPED` and continue.
