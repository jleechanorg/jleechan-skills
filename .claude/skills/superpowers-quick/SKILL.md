...
6. **Execute advice reviews (`/advice` and conditional `/web-advice`):**
   - Run `/advice` once on both the design specification and implementation plan. A bare invocation keeps `/advice` Reviewer D disabled under this wrapper's no-external-browser-review rule.
