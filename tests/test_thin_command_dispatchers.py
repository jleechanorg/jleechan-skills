...
EXTENDED_LIBRARY = COMMANDS / "extended-library"
...
class ThinCommandDispatchersTest(unittest.TestCase):
    def test_inventory_resolves_to_local_skills_with_argument_forwarding(self):
        ...
        result = validate_commands(COMMANDS, SKILLS, top_level_only=True)
        self.assertEqual([], result.errors, "\n".join(result.errors))
        self.assertGreaterEqual(result.command_count, 44)
        self.assertEqual(result.command_count, result.dispatcher_count)
    ...
    def test_factory_is_a_thin_local_dispatcher(self):
        ...
        self.assertTrue((SKILLS / "dark-factory" / "SKILL.md").is_file())
    ...
    def test_dispatcher_count_counts_valid_commands_not_errors(self):
        ...
        with TemporaryDirectory() as temp:
            ...
            result = validate_commands(commands, root / "skills", top_level_only=True)
    ...
    def test_extended_library_commands_remain_intact_and_distinct(self):
        self.assertTrue(EXTENDED_LIBRARY.is_dir())
        extended_files = list(EXTENDED_LIBRARY.glob("*.md"))
        self.assertGreater(len(extended_files), 100)
        # Check a sample of extended-library commands to ensure they remain intact
        for sample in ("parallel.md", "secondo.md", "perp.md", "thinku.md"):
            with self.subTest(sample=sample):
                sample_path = EXTENDED_LIBRARY / sample
                self.assertTrue(sample_path.is_file(), f"missing extended command: {sample_path}")
